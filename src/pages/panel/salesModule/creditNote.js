...
import dbCreditNote from 'models/CreditNote';
...
import ReactToPrint from 'react-to-print';
  ...
  const CreditNote = ({ dbVouchers, dbProducts, dbContacts, dbEmployees, dbTaxRate, dbProject }) => {
    ...
    const submit = async(e)=>{
      ...
      const data = { phoneNo, email, city, address, project, dueDate, inputList, name,  memo, journalDate, journalNo, fullAmount, fullTax, totalAmount, attachment, path:'CreditNote' };
          ...
          <div className="px-4 sm:px-0 flex">
            ...
            <button 
              onClick={()=>{
                ...
                setJournalNo(`CN-${dbVouchers.length === 0 || !dbVouchers[dbVouchers.length - 1].journalNo
                  ? dbVouchers.length + 1
                  : parseInt(dbVouchers[dbVouchers.length - 1].journalNo.slice(3)) + 1}`)


                setInputList([
                  {journalNo : `CN-${dbVouchers.length === 0 || !dbVouchers[dbVouchers.length - 1].journalNo
                    ? dbVouchers.length + 1
                    : parseInt(dbVouchers[dbVouchers.length - 1].journalNo.slice(3)) + 1}`, 
                ...
                <table ref={componentRef} className="w-full text-sm text-left text-gray-500 ">
                  ...
                  <tbody>
                    ...
                    return <tr key={index} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="p-1">
                        <div className='text-sm'>{item.inputList[0].products}</div>
                      </td>
                      <td className="p-1">
...
export async function getServerSideProps() {
  ...
  let dbVouchers = await dbCreditNote.find()
